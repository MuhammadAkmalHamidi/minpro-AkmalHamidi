...
export const LatesBlog = () => {
    ...
    const latesBlog = async () => {
        try {
            const response = await Axios.get(`https://minpro-blog.purwadhikabootcamp.com/api/blog?id_cat=3&sort=ASC&page=1/`, data)
    ...
    return (
        <Box>
            <Flex w={"1200px"}>
                <Heading bgClip={"text"} bgGradient={"linear(to-r, blue.200, purple)"}>Lates Blog</Heading>
                <Box ml={"10px"} mt={"35px"} w={"1000px"} h={"2px"}bgGradient={"linear(to-r, blue.200 , purple)"}></Box>
            </Flex>
